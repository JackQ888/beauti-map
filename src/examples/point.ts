...
export default {
    name: '标注点',
    description: 'Point 标注 + SpriteText 文字',
    async setup(map: any) {
        ...
        backgroundArea.setProps({
            border: { color: '#ccc' },
            background: { color: 'rgba(34,128,178,0.1)' }
        })
        ...
        section.setProps({
            color: [[0, 'rgba(75,144,196,0.5)'], [0.5, 'rgba(0,0,0,0.5)'], [1, '#0092ff']],
            thickness: 50,
        ...
        boundary.setProps({
            ...
            insetShadow: { enable: true, shadowBlur: 100, shadowColor: '#00fff2' }
        })
        boundary.height = 50
        ...
        hoverSection.setProps({
            effect: 0.5,
            color: [[0, 'rgba(55,42,0,0.4)'], [1, 'rgba(255,255,5,0.8)']],
            thickness: 0
        })
        hoverSection.height = 50

        // SpriteText
        let spriteText = map.createChild('SpriteText')
        spriteText.setProps({
            offset: { x: 100, y: 100 },
            text: {
                align: { vertical: 'bottom', horizontal: 'center' },
                font: { fontSize: 18, fontStyle: 'normal', fontWeight: 'normal', fontFamily: 'DingTalk', color: '#fff' },
                shadow: { enable: true, shadowBlur: 5, shadowColor: '#000', shadowOffsetX: 0, shadowOffsetY: 5 }
            },
            backdrop: { enable: false }
        })
        spriteText.height = 50

        // Point
        let point = map.createChild('Point')
        point.setData([
            { point: '650000', radius: 40, color: '#ff0' },
            { point: '150000', radius: 80, color: '#aff' },
            { point: '330000', radius: 80, color: '#f00' }
        ])
        point.height = 50

        map.addChild(txtPanel, circlePanel2, gridPanel, trace, circlePanel, section, backgroundArea, boundary,
            effectLight, hoverSection, hoverArea, hoverBoundary, spriteText, point)
